...
const OptionItemText = props => {
    const {id, title, onChange, onKeyUp, value, autoFocus, ...rest} = props;
    return (
        <Item>
            <Box width="100px" mr="10px">{title}</Box>
            {/* {value ?  */}
              <CustomInput fullWidth variant="standard" onChange={onChange} onKeyUp={onKeyUp} value={value} id={id} autoFocus={autoFocus} size="small"></CustomInput>
